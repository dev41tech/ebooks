...
import { env, identity, initialize, getDatabase } from './test-runtime.mjs';
const root = path.resolve(import.meta.dirname, '..');
const output = path.join(root, 'tests', '.generated');
const routes = {};
...
before(async()=>{
 ...
 await build({entryPoints:[path.join(root,'tests/pg-runtime.ts')],outfile:path.join(output,'pg-runtime.mjs'),bundle:true,platform:'node',format:'esm',packages:'external'});
 const module=await import(pathToFileURL(path.join(output,'pg-runtime.mjs')));
 runtime=new PGlite();
 for(const name of (await readdir(path.join(root,'drizzle'))).filter(x=>x.endsWith('.sql')).sort())await runtime.exec(await readFile(path.join(root,'drizzle',name),'utf8'));
...
after(async()=>{await runtime?.close();await rm(output,{recursive:true,force:true});});
...
test('bulk uploads enter review together without invented authors or automatic publication',async()=>{
 ...
 for(const row of rows.results){assert.equal(row.published_book_id,null);assert.equal(row.rights_confirmed,false);assert.ok(row.storage_key);}
});
...
test('editorial check is read-only and publication stores automatic warnings',async()=>{
 ...
 const stored=(await db.prepare('SELECT validation_errors AS value FROM staging_books WHERE id=?').bind(id).first()).value;assert.ok(stored.length>0);
...
test('pilot backup is master-only, contains product data and restores to isolated PostgreSQL',async()=>{
 ...
 assert.equal(snapshot.format,'sambu-postgres-backup-v1');
 ...
 assert.deepEqual(files['assets/imports/direct/valid.epub'],epub);
 const restored=new PGlite();
 try {
  for(const name of (await readdir(path.join(root,'drizzle'))).filter(x=>x.endsWith('.sql')).sort())await restored.exec(await readFile(path.join(root,'drizzle',name),'utf8'));
  for(const [table,data] of Object.entries(snapshot.tables))for(const row of data){
   const keys=Object.keys(row);await restored.query(`INSERT INTO "${table}" (${keys.map(k=>`"${k}"`).join(',')}) VALUES (${keys.map((_,i)=>'$'+(i+1)).join(',')})`,Object.values(row).map(v=>v!==null&&typeof v==='object'?JSON.stringify(v):v));
  }
  assert.equal((await restored.query('SELECT count(*)::int AS n FROM books')).rows[0].n,snapshot.tables.books.length);
...
test('chunked upload validates bytes, isolates owners and completes idempotently',async()=>{
 identity.email=admin;const init=await routes.uploads.POST(payload({action:'init',fileName:'teste.epub',size:epub.length}));assert.equal(init.status,200);
 const {uploadId}=await init.json();
 const part=()=>new Request(`https://sambu.test/api?uploadId=${uploadId}&part=0`,{method:'PUT',body:epub});
 identity.email=reader;assert.equal((await routes.uploads.PUT(part())).status,403);
 identity.email=admin;
 assert.equal((await routes.uploads.POST(payload({action:'part',uploadId,part:0,data:btoa('short')}))).status,400);
 assert.equal((await routes.uploads.PUT(part())).status,200);
 const done={action:'complete',uploadId,totalParts:1,size:epub.length};
 const response=await routes.uploads.POST(payload(done));assert.equal(response.status,200);
 const file=await response.json();assert.deepEqual(new Uint8Array(await (await env.BUCKET.get(file.storageKey)).arrayBuffer()),epub);
 assert.equal((await env.BUCKET.head(file.storageKey)).customMetadata.owner,admin);
 assert.equal((await routes.uploads.POST(payload(done))).status,200);
});

test('Supabase adapter enforces range/etag and persists ownership in PostgreSQL',async()=>{
 const outfile=path.join(output,'storage.mjs'),runtimePath=path.join(root,'tests/test-runtime.mjs');
 await build({entryPoints:[path.join(root,'db/storage.ts')],outfile,bundle:true,platform:'node',format:'esm',packages:'external',plugins:[{name:'test-db-only',setup(b){b.onResolve({filter:/^\.\/sql$/},()=>({path:runtimePath,external:true}));}}]});
 const {bucket}=await import(pathToFileURL(outfile));
 const original=globalThis.fetch;
 const previous={...process.env};process.env.SUPABASE_URL='https://supabase.example.test';process.env.SUPABASE_SERVICE_ROLE_KEY='test-only';
 let mode='normal';const calls=[];
 globalThis.fetch=async(url,init)=>{
  calls.push({url,init});
  if(init.method==='POST')return Response.json({Key:'sambu/test.epub'});
  if(mode==='missing')return new Response(null,{status:404});
  if(mode==='ignored')return new Response('abc',{headers:{etag:'"original"','content-length':'3'}});
  const headers={etag:mode==='changed'?'"changed"':'"original"','content-length':'3','content-type':'application/epub+zip'};
  return new Response(init.method==='HEAD'?null:'abc',{status:init.headers.Range?206:200,headers});
 };
 try{
  await bucket.put('test.epub','abc',{customMetadata:{owner:admin}});
  assert.equal((await bucket.head('test.epub')).customMetadata.owner,admin);
  assert.equal(await (await bucket.get('test.epub',{range:{offset:2,length:3}})).text(),'abc');
  assert.equal(calls.at(-1).init.headers.Range,'bytes=2-4');
  mode='ignored';await assert.rejects(()=>bucket.get('test.epub',{range:{offset:2,length:3}}),/storage_range_not_supported/);
  mode='changed';assert.equal(await bucket.get('test.epub',{onlyIf:{etagMatches:'original'}}),null);
