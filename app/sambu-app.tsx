...
export default function SambuApp({user}:{user:User}) {
  ...
  return <div className="app-shell beta-app">
    {view!=="reader"&&<header><button className="brand community-brand" onClick={()=>navigate("home")} aria-label="Início Sambu"><img src="/sambu-comunidade.png" alt="Sambu — Comunidade de leitura" width="1536" height="1024"/></button><nav aria-label="Navegação principal">{NAV.map(item=><button key={item.id} className={view===item.id?"active":""} onClick={()=>navigate(item.id)}>{item.label}</button>)}{user?.admin&&<button onClick={()=>navigate("admin")}>Administração</button>}</nav><button className="outline" onClick={()=>navigate("profile")}>{user?"Minha conta":"Entrar"}</button></header>}
