...
function Profile({user,notify}:{user:User;notify:(s:string)=>void}) {
  ...
  if(user.temporaryAdmin)return <main className="page"><h1>Acesso de teste</h1><p>A leitura e a importação de novos livros estão temporariamente abertas, sem cadastro ou senha, até o término do período de teste.</p><p>Os favoritos e o progresso deste teste são compartilhados entre os visitantes. As contas pessoais voltam a ser usadas quando o login for reativado.</p><div className="feedback-actions"><a className="primary" href="/?view=admin">Abrir administração</a><a className="outline" href="/?view=catalog">Explorar livros</a></div></main>;
