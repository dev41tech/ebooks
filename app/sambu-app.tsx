"use client";
import { readingBlocks } from "./lib/reader-blocks";
...
type Section = { id: string; title?:string; body: string[]; minutes: number };
...
export default function SambuApp({user}:{user:User}) {
  ...
  const startReading = useCallback(async(book:Book)=>{
    ...
    try {
      ...
      else {const data=await requestJson(`/api/catalog/content?id=${encodeURIComponent(book.id)}`,{signal:controller.signal});if(!data.chapters?.length)throw new Error("Este livro não possui conteúdo legível.");setSections(data.chapters);navigate("reader",book);}
    }catch(e){notify(controller.signal.aborted?"A abertura demorou mais que o esperado. Verifique sua conexão e tente novamente.":(e as Error).message);}finally{clearTimeout(timeout);readingLock.current=false;setOpeningBookId(null);}
  ...
  return <div className="app-shell beta-app">
    {view!=="reader"&&<header><button className="brand community-brand" onClick={()=>navigate("home")} aria-label="Início Sambu"><img src="/sambu-comunidade-horizontal.png" alt="Sambu — Comunidade de leitura" width="1984" height="792"/></button><nav aria-label="Navegação principal">{NAV.map(item=><button key={item.id} className={view===item.id?"active":""} onClick={()=>navigate(item.id)}>{item.label}</button>)}{user?.admin&&<button onClick={()=>navigate("admin")}>Administração</button>}</nav><button className="outline" onClick={()=>navigate("profile")}>{user?"Minha conta":"Entrar"}</button></header>}
    {view!=="reader"&&<div className="beta-banner">Versão beta · entre com ChatGPT para ler gratuitamente</div>}
...
function Reader({book,sections,initial,theme,font,preference,onSave,onBack}:{book:Book;sections:Section[];initial:Location;theme:string;font:number;preference:(t:string,f:number)=>void;onSave:(id:string,l:Location)=>Promise<Location>;onBack:()=>void}) {
  ...
  const blocks=useMemo(()=>readingBlocks(sections),[sections]);
  const [state,setState]=useState(""),[pdfPage,setPdfPage]=useState(Math.max(1,initial.position));
  const [current,setCurrent]=useState(initial);const currentRef=useRef(initial);const ready=useRef(false);const dirty=useRef(false);const intent=useRef(false);const generation=useRef(0);const timer=useRef<ReturnType<typeof setTimeout>|null>(null);
  const queue=useRef<Promise<boolean>>(Promise.resolve(true));const revision=useRef(initial.revision??0);const syncing=useRef(false);
  const pdf=book.format?.toUpperCase().includes("PDF");
  const apply=useCallback((location:Location)=>{ready.current=false;intent.current=false;dirty.current=false;generation.current++;if(timer.current)clearTimeout(timer.current);revision.current=location.revision??0;currentRef.current=location;setCurrent(location);if(pdf)setPdfPage(Math.max(1,location.position));else requestAnimationFrame(()=>{document.getElementById(`paragraph-${Math.min(location.position,Math.max(0,paragraphs.length-1))}`)?.scrollIntoView();requestAnimationFrame(()=>{ready.current=true;});});},[pdf,paragraphs.length]);
  const persist=useCallback((location:Location):Promise<boolean>=>{
    ...
    const job=queue.current.catch(()=>false).then(async()=>{
      ...
      setState("Salvando posição…");
      try{const saved=await onSave(book.id,{...location,revision:revision.current});revision.current=saved.revision??0;if(currentRef.current.position===location.position&&currentRef.current.progress===location.progress){dirty.current=false;currentRef.current=saved;setCurrent(saved);}setState("Posição sincronizada com sua conta");return true;}
      catch(error){const remote=(error as Error & {location?:Location}).location;if(remote){apply(remote);setState("Retomamos a posição salva no outro dispositivo.");}else setState("Sem sincronização. Tente salvar novamente antes de trocar de dispositivo.");return false;}
    });queue.current=job;return job;
  },[book.id,onSave,apply]);
  useEffect(()=>{
    apply(initial);
    const input=()=>{intent.current=true;};
    const scroll=()=>{if(pdf||!ready.current||!intent.current||document.visibilityState!=="visible")return;const elements=Array.from(document.querySelectorAll<HTMLElement>('[data-reader-position]'));let position=0;for(const element of elements){if(element.getBoundingClientRect().top<=145)position=Number(element.dataset.readerPosition);else break;}if(position===currentRef.current.position)return;const location={position,progress:currentRef.current.progress===100?100:Math.min(99,Math.max(1,Math.round(position/Math.max(1,paragraphs.length)*100))),revision:revision.current};currentRef.current=location;setCurrent(location);dirty.current=true;setState("Salvando posição…");if(timer.current)clearTimeout(timer.current);timer.current=setTimeout(()=>persist(location),600);};
    const refresh=async()=>{if(document.visibilityState!=="visible"||syncing.current)return;syncing.current=true;try{await persist(currentRef.current);if(dirty.current)return;const data=await requestJson("/api/progress",{cache:"no-store"});const latest=data.locations?.[book.id]||{position:0,progress:0,revision:0};if((latest.revision??0)!==revision.current){apply(latest);setState("Leitura sincronizada com o outro dispositivo.");}}catch{setState("Não foi possível sincronizar. Verifique sua conexão.");}finally{syncing.current=false;}};
    const hide=()=>{if(document.visibilityState==="hidden"){if(timer.current)clearTimeout(timer.current);void persist(currentRef.current);}else void refresh();};
    const leave=()=>{if(timer.current)clearTimeout(timer.current);void persist(currentRef.current);};
