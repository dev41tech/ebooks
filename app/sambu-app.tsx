...
export default function SambuApp({user}:{user:User}) {
  ...
  const startReading = useCallback(async(book:Book)=>{
    ...
    setReadingBusy(true);
    try {
      const progress=await requestJson("/api/progress",{cache:"no-store"});setLocations(progress.locations||{});
  ...
  return <div className="app-shell beta-app">
    ...
    {view==="home"&&<BetaHome books={books} user={user} loading={loading} error={error} retry={load} favorites={favorites} locations={locations} readingBusy={readingBusy} onOpen={b=>navigate("detail",b)} onRead={startReading} onFavorite={favorite} onCatalog={g=>{if(g)rememberSearch(g);setQuery("");setGenre(g||"Todos");navigate("catalog");}} onLibrary={()=>navigate("library")} onSearch={q=>{rememberSearch(q);setQuery(q);setGenre("Todos");navigate("catalog");}}/>}
...
function BetaHome({books,user,loading,error,retry,favorites,locations,readingBusy,onOpen,onRead,onFavorite,onCatalog,onLibrary,onSearch}:{books:Book[];user:User;loading:boolean;error:string;retry:()=>unknown;favorites:string[];locations:Record<string,Location>;readingBusy:boolean;onOpen:(b:Book)=>void;onRead:(b:Book)=>void;onFavorite:(id:string)=>void;onCatalog:(genre?:string)=>void;onLibrary:()=>void;onSearch:(q:string)=>void}) {
