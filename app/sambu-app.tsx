...
export default function SambuApp({user}:{user:User}) {
  ...
  return <div className="app-shell beta-app">
    ...
    {view==="home"&&<BetaHome books={books} user={user} loading={loading} error={error} retry={load} favorites={favorites} locations={locations} readingBusy={readingBusy} onOpen={b=>navigate("detail",b)} onRead={startReading} onFavorite={favorite} onCatalog={g=>{setQuery("");setGenre(g||"Todos");navigate("catalog");}} onLibrary={()=>navigate("library")} onSearch={q=>{setQuery(q);setGenre("Todos");navigate("catalog");}}/>}
...
function BetaHome({books,user,loading,error,retry,favorites,locations,readingBusy,onOpen,onRead,onFavorite,onCatalog,onLibrary,onSearch}:{books:Book[];user:User;loading:boolean;error:string;retry:()=>unknown;favorites:string[];locations:Record<string,Location>;readingBusy:boolean;onOpen:(b:Book)=>void;onRead:(b:Book)=>void;onFavorite:(id:string)=>void;onCatalog:(genre?:string)=>void;onLibrary:()=>void;onSearch:(q:string)=>void}) {
  ...
  return <main className="r3-home">
    ...
    <div className="r3-home-content">
      ...
      {!!genres.length&&<section className="r3-home-section"><div className="r3-section-heading"><div><p className="eyebrow">ESCOLHA SEU MOMENTO</p><h2>Por onde vamos começar?</h2></div></div><div className="r3-genre-grid">{genres.map((g,i)=><button className={`r3-genre r3-genre-${i%4}`} key={g} onClick={()=>onCatalog(g)}><span>{String(i+1).padStart(2,"0")}</span><strong>{g}</strong><small>Explorar livros →</small></button>)}</div></section>}
