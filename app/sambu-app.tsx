...
function Profile({user,notify}:{user:User;notify:(s:string)=>void}) {
  ...
  return <main className="page"><h1>Minha conta</h1><p>Escolha como quer ser chamado no Sambu.</p><p>{user.email}</p><p>{user.admin?"Administrador":user.participant?"Participante do beta":"Conta identificada · convite pendente"}</p><form className="beta-form" onSubmit={async e=>{e.preventDefault();setBusy(true);try{await requestJson("/api/profile",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({displayName:name})});notify("Nome salvo com sucesso. Sua biblioteca está pronta para você.");}catch(e){notify((e as Error).message);}finally{setBusy(false);}}}><label>Nome de exibição<input value={name} required maxLength={80} onChange={e=>setName(e.target.value)}/></label><button className="primary" disabled={busy}>{busy?"Salvando…":"Salvar nome"}</button></form><p>O beta é gratuito. Nenhuma assinatura ou cobrança é iniciada aqui.</p>{user.participant&&<p><a className="primary" href="/?view=catalog">Explorar livros e começar a ler</a></p>}<a href={serviceUrl("/signout-with-chatgpt?return_to=%2F")}>Sair da conta</a></main>;
}
