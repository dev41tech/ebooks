...
export type User = { name: string; email: string; admin: boolean; participant: boolean; adminTrial?:boolean; temporaryAdmin?:boolean } | null;
...
export default function SambuApp({user}:{user:User}) {
  ...
  return <div className="app-shell beta-app">
    ...
    {view==="reader"&&selected&&<Reader key={selected.id} temporaryAccess={user?.temporaryAdmin} book={selected} initialPage={readerPage} initial={locations[selected.id]||{position:0,progress:0}} theme={theme} font={font} preference={preference} onSave={save} onBack={()=>navigate("detail",selected)}/>}
    {view==="guide"&&<PilotGuide signedIn={!!user} temporaryAccess={user?.temporaryAdmin} onCatalog={()=>navigate("catalog")} onProfile={()=>navigate("profile")}/>}
    {view==="profile"&&<Profile user={user} notify={notify}/>}
    {view==="admin"&&<main className="page">{user?.admin?(user.adminTrial?<><p className="beta-banner">Administração de testes · as publicações entram no acervo real.</p><Admin temporaryAccess={user.temporaryAdmin} owner={user.email} notify={notify} onChange={load}/></>:<MasterGate owner={user.email} notify={notify} onChange={load}/>):<AccessNotice user={user} admin/>}</main>}
...
function BetaHome({books,user,loading,error,retry,favorites,locations,readingBusy,openingBookId,onOpen,onRead,onFavorite,onCatalog,onLibrary,onSearch}:{books:Book[];user:User;loading:boolean;error:string;retry:()=>unknown;favorites:string[];locations:Record<string,Location>;readingBusy:boolean;openingBookId:string|null;onOpen:(b:Book)=>void;onRead:(b:Book)=>void;onFavorite:(id:string)=>void;onCatalog:(genre?:string)=>void;onLibrary:()=>void;onSearch:(q:string)=>void}) {
  ...
  return <main className="r3-home"><div className="mobile-home-intro"><p>SAMBU · COMUNIDADE DE LEITURA</p><h1>Seu momento de ler.</h1><button onClick={()=>onCatalog()}>Buscar sua próxima história <span aria-hidden="true">⌕</span></button></div>
...
function Profile({user,notify}:{user:User;notify:(s:string)=>void}) {
  ...
  const load=useCallback(async()=>{if(!user||user.temporaryAdmin)return;setLoading(true);setError("");try{const d=await requestJson("/api/profile");setName(d.profile?.displayName||user.name);setDetails(cleanReaderProfile(d.profile?.tasteProfile));}catch{setError("Não foi possível carregar seu cadastro. Tente novamente antes de editar.");}finally{setLoading(false);}},[user]);
  useEffect(()=>{void load();},[load]);
  if(!user)return <main className="page"><AccessNotice user={user}/></main>;
  if(user.temporaryAdmin)return <main className="page beta-reader-welcome"><p className="eyebrow">LEITORES BETA</p><h1>Uma biblioteca só sua.</h1><p>Crie sua conta gratuita para salvar favoritos e retomar a leitura em qualquer dispositivo.</p><a className="primary" href="/login?return_to=%2F%3Fview%3Dlibrary">Entrar ou criar conta Beta</a><p>Ao entrar, seus livros salvos e seu progresso ficam vinculados à sua conta. O histórico compartilhado do teste permanece separado.</p><a className="outline" href="/?view=admin">Administração de teste</a></main>;
...
function Admin({owner,notify,onChange,temporaryAccess=false}:{owner:string;notify:(s:string)=>void;onChange:()=>Promise<unknown>;temporaryAccess?:boolean}) {
  ...
  return <><div className="page-title"><h1>Administração do acervo</h1><p>{rows.filter(b=>b.status==="published").length} obras publicadas · {rows.filter(b=>b.status!=="published").length} fora do catálogo</p></div><div className="tabs"><button disabled={importing} onClick={()=>{setTab("catalog");reload();}}>Acervo</button><button disabled={importing} onClick={()=>setTab("imports")}>Importar e revisar</button><button disabled={importing} onClick={()=>setTab("bulk")}>Subir ebooks em lote</button><button disabled={importing} onClick={()=>setTab("beta")}>{temporaryAccess?"Importar acervo do backup":"Acompanhar beta"}</button></div>{error&&<div role="alert"><p>{error}</p><button onClick={reload}>Tentar novamente</button></div>}
    {tab==="beta"?(temporaryAccess?<CatalogTransfer/>:<BetaDashboard/>):(tab==="imports"||tab==="bulk")?<ImportCenter onBusy={setImporting} key={tab} initialMode={tab==="bulk"?"quick":"individual"} owner={owner} notify={message=>{notify(message);reload();}}/>:<>{!rows.length&&!error&&<p>Importe e revise o primeiro ebook para iniciar o acervo.</p>}<div className="admin-table-wrap catalog-admin-table"><table><thead><tr><th>Livro</th><th>Autor</th><th>Situação</th><th>Ação</th></tr></thead><tbody>{rows.map(b=><tr key={b.id}><td data-label="Livro">{b.title}</td><td data-label="Autor">{b.author}</td><td data-label="Situação">{b.status==="published"?"Publicado":"Fora do catálogo"}</td><td data-label="Ação"><button className="outline" disabled={temporaryAccess} onClick={()=>{setEditing(b);setDeleteOpen(false);setConfirmTitle("");}}>{temporaryAccess?"Somente consulta":"Editar"}</button></td></tr>)}</tbody></table></div></>}
