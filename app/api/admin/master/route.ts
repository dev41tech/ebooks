...
export async function POST(request:Request) {
  ...
  const attempt=await env.DB.prepare("INSERT INTO master_attempts(email,attempts,window_start) VALUES(?,1,?) ON CONFLICT(email) DO UPDATE SET attempts=CASE WHEN window_start<=? THEN 1 ELSE attempts+1 END, window_start=CASE WHEN window_start<=? THEN excluded.window_start ELSE window_start END RETURNING attempts").bind(user.email,now,now-windowMs,now-windowMs).first<{attempts:number}>();
