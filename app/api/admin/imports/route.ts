...
export async function PATCH(request: Request) {
  ...
  const mutations: ((tx: Awaited<ReturnType<typeof getDb>>) => PromiseLike<unknown>)[] = [];
  if (action === "publish") {
    ...
    mutations.push(tx => tx.insert(books).values({
      id: item.id,
  ...
  await db.transaction(async tx=>{for(const mutate of mutations)await mutate(tx as unknown as Awaited<ReturnType<typeof getDb>>);});
