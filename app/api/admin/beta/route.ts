...
export async function GET(){
 ...
 const [activity,failures,feedback,ratings,books,counts]=await Promise.all([
  env.DB.prepare(`SELECT COUNT(*) AS activeReaders,COALESCE(SUM(CASE WHEN days>1 THEN 1 ELSE 0 END),0) AS returningReaders FROM (SELECT user_email,COUNT(DISTINCT substr(created_at,1,10)) AS days FROM analytics_events WHERE event='reader_opened' AND created_at>=? GROUP BY user_email) AS activity_days`).bind(since).first(),
  ...
  env.DB.prepare("SELECT COUNT(*) AS reports,COALESCE(SUM(CASE WHEN status!='resolved' THEN 1 ELSE 0 END),0) AS unresolved FROM beta_feedback").first()
 ]);
