...
export const BACKUP_TABLES=['profiles','books','chapters','reading_progress','bookmarks','favorites','analytics_events','subscriptions','reviews','reading_sessions','notifications','media_assets','import_batches','staging_books','discovery_searches','beta_feedback'] as const;
...
export async function prepareBackup(){
 ...
 const schema=(await env.DB.prepare("SELECT type,name,tbl_name,sql FROM sqlite_master WHERE type IN ('table','index') AND sql IS NOT NULL ORDER BY CASE type WHEN 'table' THEN 0 ELSE 1 END").all()).results.filter(row=>BACKUP_TABLES.includes(row.tbl_name as typeof BACKUP_TABLES[number]));
 ...
 const snapshot={format:'sambu-pilot-backup-v1',createdAt:new Date().toISOString(),tables,schema,assets,excluded:['login credentials and sessions','hosting secrets','source code','unreferenced uploads','regenerable chapter caches']};
 const metadata=strToU8(JSON.stringify(snapshot));if(metadata.byteLength>10_000_000)throw new Error('backup_limit');
 return {snapshot,metadata};
}

/** ZIP store mode streams each source once; no complete ebook is buffered in the Worker. */
